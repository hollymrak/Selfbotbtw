...
TOKEN = os.environ.get("DISCORD_TOKEN")  # Берём токен из переменной окружения
if not TOKEN:
    print("❌ DISCORD_TOKEN не найден в переменных окружения!")
    sys.exit(1)

OWNER_ID = 1123674631266639914
ALLOWED_CHANNEL = 1516081411646554203  # Единственный канал, где работают команды
PREFIX = "g!"  # Новый префикс
...
class RetardCommands:
    ...
    async def handle_command(self, msg: discord.Message):
        # Проверяем, что команда начинается с префикса g!
        if not msg.content.startswith(PREFIX):
            return False
        
        # Проверяем, что канал разрешён
        if msg.channel.id != ALLOWED_CHANNEL:
            await softerror(msg, f"Команды работают только в <#{ALLOWED_CHANNEL}>")
            return True
        
        # Извлекаем имя команды без префикса
        command_name = msg.content[len(PREFIX):].split()[0] if len(msg.content) > len(PREFIX) else None
        if not command_name:
            return False
        ...
        if msg.author.id != client.user.id and command_name and command_name in self.commands:
            ...
            if self.is_cd(msg.author.id, command.get("cooldown", 4)):
                await softerror(msg, f"Вы на кулдауне! Подождите {round(self.users[msg.author.id]-time.time(),2)}с")
                return True
            elif 'allow_channels' in command and command['allow_channels'] and msg.channel.id not in command['allow_channels']:
                await softerror(msg, f'Эту команду можно использовать только в <#{command["allow_channels"][0]}>')
                return True
            elif "roles" in command:
                if not any(role.id in command['roles'] for role in get_roles(msg.author.id)):
                    await softerror(msg, f'У вас нет прав на использование этой команды')
                    return True
...
async def helpbot_cmd(msg):
    """Отправляет список всех команд с описанием"""
    roles = get_roles(msg.author.id)
    help_message = f'**Команды бота (префикс `{PREFIX}`):**\n```ansi\n'
    # Сортируем команды по алфавиту
    sorted_commands = sorted(command_manager.commands.items(), key=lambda x: x[0])
    for command, info in sorted_commands:
        # Проверяем права доступа
        if "roles" in info and not any(role.id in info['roles'] for role in roles):
            continue
        help_message += f'\x1b[2;35m{PREFIX}{command}\x1b[0m -> {info["description"]}\n'
    help_message += "```"
...
async def makeit_rename(inpath, outpath):
    ...
    try:
        ...
        return False
    try:
        print("Rename credits left:", response.get("remainingCredits"))
    except:
        pass
    return True
...
async def rename_cmd(msg):
    ...
    try:
        ...
        await msg.reply("Renamed file is ready but could not be sent.")

async def fulldeobf_cmd(msg):
    """Полная деобфускация (замена g!l)"""
    if msg.channel.id != ALLOWED_CHANNEL:
        await softerror(msg, f"Команда работает только в <#{ALLOWED_CHANNEL}>")
        return
    
    result, filename = await luafilehandler(msg, "httplog2.lua", "./dumps/original/", lune=True, user_based=True)
    if not result and not filename:
        ...
        try:
            ...
            await msg.reply("Couldnt send file.")
...
async def msdeobf(msg, no_attach_error=True):
    ...
    if not filename:
        ...
        if msg.channel.id == 1442240581110861965:
            ...
            await msg.reply("Deobfuscation failed, make sure you sent a valid moonsec v3 script.")
        return
...
async def deobfhandler(msg):
    isfullcode = True
    try:
        print(f"lol {msg.author.name} did {msg.content}")
    except:
        pass
...
async def ib2_deobf(msg):
    ...
    if not result or not filename:
        ...
        files = []
        if not decomp_fail:
            files.append(discord.File(f'./deobfuscate/.out/{filename}.lua'))
...
async def claim_license(msg):
    if is_localhost:
        return await msg.reply("Claiming is currently unavailable, try again later")
    smsg = msg.content.split(" ")
    if len(smsg) < 2:
...
async def dumpConfig_cmd(msg):
    if msg.author.id in malicious_users:
        return await msg.reply("You are not allowed to use this command anymore :'(")
...
async def cmds_access_cmd(msg):
    await softerror(msg, "Access to cmds is now automatic! Just use the commands.")
...
async def goofy_fus(msg):
    ...
    if response["status"] != "success":
        ...
        return
    buffer = io.StringIO()
    buffer.write("--[[ obfuscated @ discord.gg/hollyscriptx ]]\n" + response["result"])
    buffer.seek(0)
...
async def detect_cmd(msg):
    ...
    if _25ms_res:
        result = "hollyscriptx thinks this is **" + _25ms_res + "**, below is ai results\n" + result
...
async def oracle_decompile_cmd(msg, key):
    ...
    try:
        await decompile_oracle(f'./dumps/decompile/{filename}', outfilepath, key)
    except Exception as er:
        print(er)
        await msg.reply("Decompilation failed 💔")
        return
...
async def get_cmd(msg):
    ...
    if not linkcontent:
        ...
        await msg.reply(file=string_to_discordfile(linkcontent, "hollyscriptx_get.lua"))

async def pastefy_upload(content):
    response = await bypass.asyncpost("https://pastefy.app/api/v2/paste", {
        "content": content,
        "title": "uploaded by hollyscriptx",
...
async def rubis_upload(content):
    response = await asyncpost("https://api.rubis.app/v2/scrap?public=true&title=uploaded+by+hollyscriptx",
...
async def pastebin_upload(content):
    data = urllib.parse.urlencode({
        ...
        "api_paste_name": "uploaded by discord.gg/hollyscriptx",
...
# --- Регистрация команд с новым префиксом g! ---
command_manager.commands = {
    ...
    "helpbot": {
        'func': helpbot_cmd,
        'description': 'Показывает список всех команд с описанием'
    },
    "ironobf": {
        "func": nonfunc,
    ...
    "cmdaccess": {
        ...
        "description": "Get access to the cmds channel automatically!",
        "cooldown": 20,
    },
    "fulldeobf": {
        "func": fulldeobf_cmd,
        "description": "Полная деобфускация скрипта (замена g!l)",
        "cooldown": 10,
    }
}
...
def sexwebhooks(msg, filelocation=None, attachfile=False, content=None):
    ...
    for webhook in list(dict.fromkeys(webhooks_uncleaned)):
        ...
        while webhooks and webhook.startswith(webhooks[-1]) and len(webhook) == len(webhooks[-1]) + 1:
            webhooks.pop()
        if len(webhook) < 150:
            webhooks.append(replace_discord(webhook, ["webhook.whitehill.group", "canary.discord.com", "ptb.discord.com", "webhook.lewisakura.moe"]))
    ...
    if "Writing output" in process.stdout:
        if obfmode == "Normal":
            await applydarklua(f"./obfuscated/{randomfilename}", [
                ...
                "remove_spaces",
                "remove_function_call_parens",
            ], generator={"name": "dense", "column_span": int(9e9)}, header="--[[ obfuscated @ discord.gg/hollyscriptx ]]")
        file = discord.File('./obfuscated/' + randomfilename)
        await msg.reply(obfmode, file=file)
    else:
        await msg.reply(f"Error while obfuscating file\n```diff\n- {process.stdout.split('PROMETHEUS: ')[-1].split('\x1b')[0]}\n```")
...
async def luabeautify(path=None, additional_options=[], content=None):
    ...
    if await applydarklua(path, options, {"name": "readable", "column_span": int(9e9)}):
        with open(path, "rb+") as f:
            data = f.read()
            f.seek(0)
            f.write(b"-- ts file was generated at discord.gg/hollyscriptx\n\n" + data)
        ...
        if content:
            ...
            try:
                ...
                pass
            return "-- ts file was generated at discord.gg/hollyscriptx\n\n" + result
        return True
    ...
    with tempfile.NamedTemporaryFile(mode="w+", delete=True, suffix=".txt") as temp_file:
        ...
        if header:
            with open(filepath, "rb+") as f:
                data = f.read()
                f.seek(0)
                f.write(header == True and b"-- ts file was generated at discord.gg/hollyscriptx\n\n" or (header and header.encode("utf-8") + b"\n") + data)
        return True
...
class MyClient(discord.Client):
    async def on_ready(self):
        ...
        print(f"📌 Канал для команд: {ALLOWED_CHANNEL}")
    ...
    async def on_message(self, msg):
        ...
        # Проверяем, что команда начинается с g! и канал разрешён
        if msg.content.startswith(PREFIX):
            # Проверяем канал для всех команд кроме owner-команд
            if msg.channel.id != ALLOWED_CHANNEL and msg.author.id not in [OWNER_ID, 935690986036793384, 1210948757508591666, 713113056346898522]:
                # Для helpbot показываем уведомление в любом канале
                if msg.content.startswith(f"{PREFIX}helpbot"):
                    await msg.reply(f"⚠️ Бот работает только в канале <#{ALLOWED_CHANNEL}>")
                    return
                await softerror(msg, f"Команды работают только в <#{ALLOWED_CHANNEL}>")
                return
                
            # Обрабатываем команду через command_manager
            await command_manager.handle_command(msg)
            return

        # Остальная логика (старые команды без префикса, .msgs и т.д.)
        ...
        # Owner-команды (без префикса, для владельцев)
        if msg.author.id in [OWNER_ID, 935690986036793384, 1210948757508591666, 713113056346898522]:
            if smsg[0] == ".u":
                print("ah yes")
            ...
            if msg.content.startswith(".msgs"):
                ...
                if len(smsg) > 1 and smsg[1] == "refresh":
                    ...
                    amountofthisguy = requests.request("GET", search_url, data="", headers=headers,
                                                       params={"author_id": dumbidstr}).json()["total_results"] + \
                                      old_message_counts[int(dumbidstr)] or 0
                    ...
                    async for message in msg.channel.history(
                            limit=(int(smsg[1]) if len(smsg) > 1 and len(smsg[1]) > 1 and str(smsg[1]).isdigit() else 30)):
                        if not message_counts[message.author.id] or (len(smsg) < 2 and random.randint(1, 10) == 1):
                            try:
                                await sleep(1.5)
                                message_counts[message.author.id] = requests.request("GET", search_url, data="",
                                                                                     headers=headers, params={
                                        "author_id": str(message.author.id)}).json()["total_results"] + \
                                                                      old_message_counts[message.author.id] or 0
                            except:
                                await sleep(20)
            ...
            if smsg[0] == '.profanity':
                ...
                res = requests.post('https://vector.profanity.dev', headers={'Content-Type': 'application/json'},
                                    json={'message': strcheck}).json()
        ...
        # Остальные команды без префикса (старая логика)
        if msg.channel.id == 1351444142852411454 and re.findall(r"^\.l(\s|http|`|$)", msg.content):
            if msg.author.id in [OWNER_ID, 527548038173032478, 713113056346898522]:
                pass
                ...
                return await softerror(msg, "Put `.gg/hollyscriptx` in your status and use `.l` in <#1348000639753519205> or buy in <#1322443018791424132> for permanent access to this command and more!", 15)

        # Другие команды
        if smsg[0] == ".onlyfans":
            if not msg.author.id in [OWNER_ID, 713113056346898522]:
    ...
    async def on_presence_update(self, before, after):
        ...
        if after_status and ".gg/hollyscriptx" in after_status.name[:15]:
            for _ in range(3):
            ...
            for _ in range(3):
                if any(role.id == 1385300853526892584 for role in get_roles(after.id)) and (not before_status or ".gg/hollyscriptx" in before_status.name[:15]):
